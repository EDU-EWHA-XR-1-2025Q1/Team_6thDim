...
public class yelin_Put_Controller : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            int pickCounts = UI.GetComponent<yelin_UI_Controller>().GetPickCounts();
            if (pickCounts > 0)
            {
                Throw();
                UI.GetComponent<yelin_UI_Controller>().Decrease_PickCounts();
            }
        }
    }
