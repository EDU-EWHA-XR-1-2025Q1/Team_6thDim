...
public class Dana_Put_Controller : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        {
            int pickCounts = UI.GetComponent<Dana_UI_Controller>().GetPickCounts();
            if (pickCounts > 0)
            {
                Throw();
                UI.GetComponent<Dana_UI_Controller>().Decrease_PickCounts();
            }
        }
    }
